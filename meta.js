...
import { focusedVerify, publishVerificationEvent } from "./verify.js"
...
export async function runMeta({ config, provider, task, onEvent = null, signal = null, resumeTaskId = null, segmentSteps, maxSegments, runAgent = null, deep, workers = null, pluginStartedAt = null, conversationId = null, episodeSink = null } = {}) {
  ...
  const emit = (ev) => {
    try {
      ...
      if (bus91 && (ev?.type === "VERIFICATION_FAILED" || ev?.type === "VERIFICATION_PASSED")) {
        ...
        // graph. This avoids two independent verification histories.
        publishVerificationEvent(ev, { bus: bus91, evidenceGraph: cognitionRef?.evidenceGraph ?? null })
      }
    } catch { }
  }
  ...
  bus91 = createBus({ taskId, persist: true })
  ...
  // live meta controller sees the same frozen intent, ambiguity and task
  // hypotheses as the one-shot agent. Resume remains authoritative; boot is
  ...
  const repairRetry = createRetryController({
    maxRetries: config?.agent?.repairRetry?.maxRetries ?? 3,
    baseMs: config?.agent?.repairRetry?.backoffMs ?? 1000,
    maxBackoffMs: config?.agent?.repairRetry?.maxBackoffMs ?? 10000,
    maxHistory: config?.agent?.repairRetry?.maxHistory ?? 32,
   ...
   * separate; this controller governs repeated repair strategies for a task
   * node. A failed strategy is never silently replayed.
   */
  const boundedRepair = async (repairArgs = {}) => {
    const nodeId = repairArgs.nodeId ?? state.node_id ?? "task"
    let strategy = "repair: diagnose root cause, minimal fix, verify"
    let experiment = ""
    try {
      const next = omega?.nextRepair?.()
      experiment = next?.experiment?.instruction ?? ""
      if (next?.action) strategy = `${strategy}; action=${next.action}`
      if (next?.causal?.node?.id) strategy += `; causal=${next.causal.node.id}`
    } catch {}
    const admission = repairRetry.admit({
      nodeId,
      strategy,
      reason: repairArgs.error ?? repairArgs.verification?.reason ?? "repair required",
      experiment,
    })
    emit({
      type: "REPAIR_RETRY_ADMISSION", taskId, runId: taskRunId, segmentId: repairArgs.segmentId ?? null, nodeId,
      allowed: admission.allowed, state: admission.state, failures: admission.failures, maxRetries: admission.maxRetries,
    ...
    emit({
      ...
      ok: recovered, state: outcome.state, failures: outcome.failures, maxRetries: outcome.maxRetries, fingerprint: outcome.fingerprint,
    })
    return { recovered, retryBlocked: false, admission, outcome }
  }
