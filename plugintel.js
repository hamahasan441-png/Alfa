...
/** Playbooks the model can follow without a live plugin-host. v165: they live
 *  in playbooks.js with their steps, which load_skill serves. */
export { PLUGIN_PLAYBOOKS }
...
export function formatPluginPicks({ tools = [], playbooks = [] } = {}) {
  const lines = []
  if (playbooks.length) {
    lines.push(`PLUGIN PLAYBOOKS (${playbooks.length}) — load_skill <name> gives the steps; do not spawn plugin-host:`)
