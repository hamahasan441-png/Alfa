...
console.log("== 4. the two follow-ups, root-caused before being touched ==")
{
  ...
  // ...and following the offsets actually works: each hop resumes exactly where
  // the previous read stopped, and the chain terminates at EOF. cognition.js
  // grows over time, so this follows the pagination chain however many pages it
  // needs instead of assuming a fixed length.
  let offset = Number(m[1])
  let everyHopResumes = true
  let hops = 0
  let rest = ""
  while (true) {
    rest = String(await tools.execTool(ctx, "read_file", { path: "cognition.js", offset }))
    if (!new RegExp(`^\\s*${offset}\\|`, "m").test(rest)) everyHopResumes = false
    const next = /continue with offset: (\d+)/.exec(rest)
    if (++hops > 100) break // guard: a non-terminating pager must not hang the suite
    if (!next) break
    const nextOffset = Number(next[1])
    if (nextOffset <= offset) { everyHopResumes = false; break } // guard: offset must advance
    offset = nextOffset
  }
  ok("following each offset resumes exactly where the previous read stopped", everyHopResumes && hops >= 1, rest.split("\n")[0])
