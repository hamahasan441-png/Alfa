...
console.log("== 3. the learning key identifies what was learned ==")
{
  ...
  // The stable key is the `key || id` fallback; cognition computes it (into
  // strategyId) and records the strategy under it. Match the behavior, not one
  // particular inline spelling of it.
  ok("and records under it", /ranked\[0\]\.key \|\| ranked\[0\]\.id/.test(cog) && /recordStrategy\(\{ cwd, klass, id: strategyId/.test(cog))
}
