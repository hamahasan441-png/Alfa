...
console.log("== 5. the compile cache: per version, pruned, opt-out ==")
{
  ...
  const run = (env = {}) => spawnSync(process.execPath, ["--input-type=module", "-e", `import { enableBootCache } from ${JSON.stringify(path.join(ROOT, "bootcache.js"))}; console.log(enableBootCache({ root: ${JSON.stringify(root)}, version: "198.0.0" }))`], { encoding: "utf8", env: { ...process.env, ...env } })
  const hasApi = typeof (await import("node:module")).default.enableCompileCache === "function"
  const r = run()
  if (hasApi) {
    ok("the cache is kept under <root>/<version>", r.stdout.trim() === path.join(root, "198.0.0") && fs.existsSync(path.join(root, "198.0.0")), r.stdout + r.stderr)
