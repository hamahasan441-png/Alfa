...
console.log("== 4. providers that do not stream ==")
{
  ...
  // v201: the Anthropic protocol streams too; this server answers with JSON anyway, which is read as JSON
  ok("the Anthropic protocol asks to stream too (v201), and a JSON answer is read as JSON", ra.content === "claude" && anth.bodies[0].stream === true)
  await anth.stop()
