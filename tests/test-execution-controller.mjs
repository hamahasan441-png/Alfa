...
console.log("== 4. repeated identical failure: honest FAILED after the repair budget (§9.2), never COMPLETED ==")
{
  ...
  ok("the give-up reason is recorded", String(r.task?.errors ?? JSON.stringify(r.task ?? "")).includes("GIVE_UP") || String(r.text ?? "").includes("consecutive failed segments") || String(r.text ?? "").includes("strategy already failed"), r.text?.slice(0, 120))
