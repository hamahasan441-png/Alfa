...
console.log("== the bump script rewrites a version, not a substring of one ==")
{
  ...
  // 127.0.0 → 135.0.1 rewrote every `127.0.0.1` in the tree — 55 files, every
  ...
  const cases = [
    ...
    ["1127.0.0", "1127.0.0"], ['"127.0.0"', '"135.0.1"'],
    ["v127.0.0", "v135.0.1"], ["127.0.0", "135.0.1"],
  ]
  const wrong = cases.filter(([i, w]) => i.replace(re, "135.0.1") !== w).map(([i]) => i)
