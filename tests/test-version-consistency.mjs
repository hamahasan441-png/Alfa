...
console.log("== a pin's LABEL says the same thing as the pin ==")
{
  ...
  // alone, so 14 suites still read `ok("package version is 196.x", /^196\./…)`
...
console.log("== the bump script rewrites a version, not a substring of one ==")
{
  ...
  // 127.0.0 → 196.0.0 rewrote every `127.0.0.1` in the tree — 55 files, every
  ...
  const cases = [
    ...
    ["1127.0.0", "1127.0.0"], ['"127.0.0"', '"196.0.0"'],
    ["v127.0.0", "v196.0.0"], ["127.0.0", "196.0.0"],
  ]
  const wrong = cases.filter(([i, w]) => i.replace(re, "196.0.0") !== w).map(([i]) => i)
  ...
  // v192: some x.0.0.1 addresses are real special-purpose blocks that the
  // SSRF suites use on purpose (192.0.0.1 is the IETF block, 10.0.0.1
  // private, 224/240 multicast/reserved) — the guard fired on 192.0.0.1 the
  // day the major became 192. Those exact fixtures are not corruption.
  const FIXTURES = /\b(?:10|192|224|240)\.0\.0\.1\b/g
  const bad = []
  for (const f of fs.readdirSync(path.join(FORGE, "tests")).filter((n) => n.endsWith(".mjs"))) {
    const t = fs.readFileSync(path.join(FORGE, "tests", f), "utf8").replace(FIXTURES, "")
