...
console.log("== the bump script rewrites a version, not a substring of one ==")
{
  ...
  // 127.0.0 → 133.2.0 rewrote every `127.0.0.1` in the tree — 55 files, every
  ...
  const cases = [
    ...
    ["1127.0.0", "1127.0.0"], ['"127.0.0"', '"133.2.0"'],
    ["v127.0.0", "v133.2.0"], ["127.0.0", "133.2.0"],
  ]
  const wrong = cases.filter(([i, w]) => i.replace(re, "133.2.0") !== w).map(([i]) => i)
