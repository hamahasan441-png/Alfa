...
console.log("== a pin's LABEL says the same thing as the pin ==")
{
  ...
  // alone, so 14 suites still read `ok("package version is 197.x", /^197\./…)`
...
console.log("== the bump script rewrites a version, not a substring of one ==")
{
  ...
  // 127.0.0 → 197.0.0 rewrote every `127.0.0.1` in the tree — 55 files, every
  ...
  const cases = [
    ...
    ["1127.0.0", "1127.0.0"], ['"127.0.0"', '"197.0.0"'],
    ["v127.0.0", "v197.0.0"], ["127.0.0", "197.0.0"],
  ]
  const wrong = cases.filter(([i, w]) => i.replace(re, "197.0.0") !== w).map(([i]) => i)
