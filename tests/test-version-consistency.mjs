...
console.log("== a pin's LABEL says the same thing as the pin ==")
{
  ...
  // alone, so 14 suites still read `ok("package version is 153.x", /^153\./…)`
...
console.log("== the bump script rewrites a version, not a substring of one ==")
{
  ...
  // 127.0.0 → 153.0.0 rewrote every `127.0.0.1` in the tree — 55 files, every
  ...
  const cases = [
    ...
    ["1127.0.0", "1127.0.0"], ['"127.0.0"', '"153.0.0"'],
    ["v127.0.0", "v153.0.0"], ["127.0.0", "153.0.0"],
  ]
  const wrong = cases.filter(([i, w]) => i.replace(re, "153.0.0") !== w).map(([i]) => i)
