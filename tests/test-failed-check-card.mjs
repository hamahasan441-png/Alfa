...
}
{
  ...
  // v202: a write whose last check FAILED is not proven — the status says COMPLETED_UNVERIFIED (exit code still 0)
  ok("the result file says COMPLETED_UNVERIFIED, with lastCheck", bad.result?.status === "COMPLETED_UNVERIFIED" && bad.result?.checks?.lastCheck?.passed === false && bad.code === 0, JSON.stringify(bad.result?.status))
