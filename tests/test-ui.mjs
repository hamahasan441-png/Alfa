...
// Asserts SECURED behavior (toolTarget redacts secrets); force the default
...
realLog("== render: header + dock adapt to width ==")
{
  ...
  // v198: "forge · agent · run de12 · …" (was "FORGE  AGENT  RUN-DE12  …")
  ok("wide header has all segments", wide.startsWith("forge · agent · run de12 · ") && wide.includes("VERIFYING 50%") && wide.includes("02:4") && wide.includes("openai/gpt-4o") && !wide.includes("  "))
  const mid = stripAnsi(renderHeader(s, 55, o))
  ok("header drops provider/elapsed before mode/state", mid.startsWith("forge · agent") && mid.includes("VERIFYING") && !mid.includes("openai"))
...
`
  ...
  ok("pty: result + COMPLETED summary", r2.text.includes("TOOL RESULT RECEIVED") && /✓ COMPLETED\s+·\s+2 steps • 1 tool calls/.test(r2.text))
