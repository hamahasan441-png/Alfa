...
console.log("== the sweep actually sweeps (end to end) ==")
{
  ...
  if (!fs.existsSync(dist)) {
    ...
    mk("199.0.0-old1", 1000), mk("199.0.0-old2", 2000)
    mk("199.0.0-old3", 3000), mk("199.0.0-old4", 4000)
    fs.mkdirSync(path.join(rt, "199.0.0-interrupted"), { recursive: true })
    try {
      ...
      ok("the two oldest complete trees are gone",
        !left.includes("199.0.0-old1") && !left.includes("199.0.0-old2"), left.join(","))
      ok("the two newest complete trees are kept",
        left.includes("199.0.0-old3") && left.includes("199.0.0-old4"), left.join(","))
      // An interrupted tree has no marker: another process may be mid-write
      // into it, so it is never a candidate.
      ok("an interrupted tree is left alone", left.includes("199.0.0-interrupted"), left.join(","))
