...
console.log("== I. package + documentation truth ==")
{
  ...
  eqv("ADV: package version is 172.0.0", pkg.version, "172.0.0")
  ...
  // v140: legacy PACKAGE_INFO.txt is optional in the shipped-root layout.
  // If a parent fixture exists, it must agree with the canonical version;
  // otherwise do not fail the release for an intentionally absent legacy file.
  const legacyInfo = new URL("../../PACKAGE_INFO.txt", import.meta.url)
  if (fs.existsSync(legacyInfo)) {
    const info = fs.readFileSync(legacyInfo, "utf8")
    ok("ADV: PACKAGE_INFO agrees with current version", info.includes(pkg.version) || !/v\d+\.\d+\.\d+/.test(info.split("\n")[0] || ""))
  } else {
    ok("ADV: legacy PACKAGE_INFO.txt is optional", true)
  }
  ok("ADV: root README claims 172.0.0 + 30 tools", /172\.0\.0/.test(fs.readFileSync(new URL("../README.md", import.meta.url), "utf8")) && /30 tools/.test(fs.readFileSync(new URL("../README.md", import.meta.url), "utf8")))
}
