...
console.log("== which lessons a run judged ==")
{
  ...
  // v198: each re-application is judged by its own check (was: only the latest)
  eq("each re-application is judged: a check between two runs of it judges the first", run({ commands: ["node setup.js", "x", "node setup.js"], commandChecks: [chk(true, 1), chk(false, 3)] }).map((o) => o.worked), [true, false])
  eq("…and the second, never checked, says nothing", run({ commands: ["node setup.js", "x", "node setup.js"], commandChecks: [chk(true, 1)] }).map((o) => o.worked), [true])
