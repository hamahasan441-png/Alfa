...
{
  const broken = []
  // Blank comments and single-quoted / template-literal string CONTENTS before
  // scanning, so an import-shaped substring inside FIXTURE source a suite writes
  // via fs.writeFileSync (e.g. `'import { b } from "./b.js"'`) is not mistaken
  // for a real import of the suite itself. Real forge imports use double-quoted
  // specifiers at module top level, so they survive this (the string-literal
  // lesson selfaudit.js documents).
  const stripFixtureLiterals = (s) => s
    .replace(/\/\*[\s\S]*?\*\//g, " ")            // block comments
    .replace(/(^|[^:"'`])\/\/[^\n]*/g, "$1")      // line comments (leaves http:// alone)
    .replace(/'(?:\\.|[^'\\])*'/g, "''")          // single-quoted strings
    .replace(/`(?:\\.|[^`\\])*`/g, "``")          // template literals
  for (const f of testFiles.filter((x) => x.endsWith(".mjs"))) {
    const src = stripFixtureLiterals(fs.readFileSync(f, "utf8"))
