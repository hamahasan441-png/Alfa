...
console.log("== which entries are rules ==")
{
  ...
  ok("framed as rules to follow", /^USER RULES \(the user's standing instructions — follow them unless the current task explicitly says otherwise\):/.test(block), block)
...
console.log("== end to end: real headless runs ==")
{
  ...
  ok("…under USER RULES", p1.includes("USER RULES (the user's standing instructions"))
