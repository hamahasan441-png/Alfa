...
// Asserts SECURED behavior (private MCP opt-in gating); force the default
// security-on contract regardless of an ambient CI FORGE_SECURITY_MODE=off.
...
console.log("== 13. transport is chosen by spec shape ==")
{
  ...
  // v179: configuredServers lives in mcpconfig.js (mcp.js re-exports it)
  const cfgSrc = fs.readFileSync(new URL("../mcpconfig.js", import.meta.url), "utf8")
  ok("url-only servers are configurable", /\(s\.command \|\| s\.url\)/.test(cfgSrc))
  ok("…and a url-only server is listed", (await import("../mcp.js")).configuredServers({ mcp: { servers: { remote: { url: "https://example.com/mcp" } } } }).length === 1)
...
console.log("== 19. the test runner itself is memory-aware (the SIGKILL fix) ==")
{
  ...
  ok("concurrency is derived by the dedicated test safety policy", /testConcurrency\(/.test(runner) && !/\|\| 4\)/.test(runner))
  ...
  ok("the headroom wait is bounded (never a deadlock)", /waited < 60000/.test(runner))
