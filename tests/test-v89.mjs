...
console.log("== 7. parallel test runner knob ==")
{
  ...
  // The machine profile (cores + available memory) is computed by
  // resourceProfile() and fed to the test-runner-policy testConcurrency() —
  // the default is derived, never a hardcoded constant.
  ok("default concurrency is derived from the machine, not hardcoded", /resourceProfile\(/.test(src) && /testConcurrency\(/.test(src))
