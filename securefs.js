...
/**
 * Serialize a read-modify-write transaction on a Forge state file across
 * processes. Atomic rename protects readers from torn writes, but it cannot
 * prevent two writers from loading the same old snapshot and losing one
 * another's update. A small mkdir-based lock closes that gap without adding a
 * dependency. Stale locks are reclaimed only when the owner PID is dead and
 * the lock is older than the grace period.
 */
export function withStateFileLock(file, fn, {
  timeoutMs = 15000,
  pollMs = 20,
  staleMs = 60000,
} = {}) {
  ...
  while (true) {
    try {
      ...
      if (Date.now() >= deadline) {
        const err = new Error(`timed out acquiring state lock: ${lock}`)
        err.code = "ELOCKTIMEOUT"
        throw err
      }
      // Atomics.wait is synchronous and does not require a timer/event-loop
      // dependency; this helper is intentionally for short state transactions.
      try { Atomics.wait(new Int32Array(new SharedArrayBuffer(4)), 0, 0, Math.max(1, pollMs)) } catch {}
    }
  }
}
