...
export async function assertFetchableUrl(url, opts = {}) {
  if (!securityEnabled() && opts.allowPrivate !== false) return { ok: true }
...
export async function pinnedFetch(url, opts = {}) {
  ...
  const securityOn = securityEnabled()
  const hops = []
  ...
  for (let hop = 0; hop <= maxRedirects; hop++) {
    ...
    const hopAllowsPrivate = !securityOn || allowPrivate === true || (allowPrivate === "first-hop" && hop === 0)
