...
import { recordVerificationEvent } from "./evidence-graph.js"
...
export function verificationEvent({ passed = false, command = null, exitCode = null, evidence = null, taskId = null, runId = null, nodeId = null, verificationId = null, files = [], reason = null } = {}) {
  const ok = passed === true
  return {
    type: ok ? "VERIFICATION_PASSED" : "VERIFICATION_FAILED",
    taskId: taskId == null ? null : String(taskId),
    runId: runId == null ? null : String(runId),
    nodeId: nodeId == null ? null : String(nodeId),
    verificationId: verificationId == null ? null : String(verificationId),
    command: command == null ? null : String(command).slice(0, 300),
    exitCode: Number.isFinite(Number(exitCode)) ? Number(exitCode) : null,
    evidence: evidence == null ? null : String(evidence).slice(0, 800),
    reason: reason == null ? (ok ? null : String(evidence ?? "verification failed").slice(0, 500)) : String(reason).slice(0, 500),
    files: Array.isArray(files) ? files.slice(0, 32).map(String) : [],
    at: Date.now(),
  }
}

export function publishVerificationEvent(event, { bus = null, evidenceGraph = null, sender = "verify", receiver = "core" } = {}) {
  const ev = event?.type ? event : verificationEvent(event)
  try { if (evidenceGraph) recordVerificationEvent(evidenceGraph, ev) } catch {}
  if (bus?.send) {
    try {
      bus.send({ sender, receiver, message_type: ev.type, content: ev.reason ?? ev.evidence ?? ev.type, task_id: ev.taskId, node_id: ev.nodeId, evidence_refs: ev.verificationId ? [ev.verificationId] : [], file_refs: ev.files, priority: ev.type === "VERIFICATION_FAILED" ? 2 : 1, requires_action: ev.type === "VERIFICATION_FAILED" })
    } catch {}
  }
  return ev
}
...
export async function runVerification(plan, { cwd = process.cwd(), timeoutMs = DEFAULT_TIMEOUT_MS } = {}) {
  ...
  out.event = verificationEvent({ passed: out.ok, evidence: out.summary, taskId: plan?.taskId, runId: plan?.runId, nodeId: plan?.nodeId, verificationId: plan?.verificationId, files: plan?.files ?? [] })
  return out
}
