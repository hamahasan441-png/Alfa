...
function writeRegistry(root, entries) {
  try {
    ...
    // Crash-safe registry update: never truncate the live registry and leave
    // invalid JSON if the process dies during serialization/write. Rename is
    ...
    // Best-effort directory flush: some filesystems/platforms do not expose
    // directory fsync, but when they do we also persist the rename itself.
    try {
