...
async function main() {
  ...
  switch (cmd) {
    ...
    case "supervise": {
      // The supervisor is deliberately outside the Forge child. SIGKILL cannot
      // be handled by Forge, so recovery must be owned by this parent process.
      const childArgs = positional.slice(1)
      if (!childArgs.length) {
        err('usage: forge supervise agent "<task>" [--deep]  (or: forge supervise -- agent "<task>")')
        process.exit(1)
        return
      }
      const { supervise } = await import("./supervisor.js")
      const result = await supervise(childArgs, { cwd: process.cwd() })
      if (JSON_OUT) { emitJson(result); return }
      if (result.ok) {
        if (result.restarts) console.log(dim(`supervisor: Forge recovered after ${result.restarts} restart(s)`))
        return
      }
      err(`supervisor: Forge stopped after ${result.restarts} restart(s) — last=${result.reason}`)
      process.exit(1)
      return
    }
    case "agent": {
    ...
    // Alpha-grade deterministic integration benchmark. This exercises the real
    // cognitive state/evidence/learning/alpha wiring without pretending to
    // measure live-model coding ability. Use `forge eval` for provider E2E.
    case "intelligence": {
      const { runIntelligenceBenchmark } = await import("./intelligence-benchmark.js")
      if (flags.list === true || positional[1] === "list") {
        if (JSON_OUT) { emitJson({ version: VERSION, benchmark: "forge-intelligence-integration", cases: 10, provider: false }); return }
        console.log(bold(`FORGE INTELLIGENCE v${VERSION}`) + dim("  10 deterministic integration cases, no live model"))
        return
      }
      const report = runIntelligenceBenchmark({ keep: flags.keep === true })
      if (JSON_OUT) { emitJson(report); return }
      console.log(bold(`FORGE INTELLIGENCE v${VERSION}`) + dim("  provider-free integration benchmark"))
      console.log(`  cases=${report.cases} transitions=${report.stateTransitions} evidence=${report.evidenceNodes}`)
      console.log(`  learning=${report.learningSamples} strategySamples=${report.learnedStrategySamples}`)
      console.log(`  goalDrift=${report.goalDriftDetected} stuck=${report.stuckDetected} elapsed=${report.elapsedMs.toFixed(1)}ms`)
      console.log(dim("  This is not a live-model success rate; use `forge eval` for provider E2E."))
      return
    }
    // Live-provider agent benchmark. Honest by construction: with no usable
    // provider it reports NOT_RUN (exit 2) instead of fabricating a score. The
    // module is also runnable standalone (`node agent-benchmark.js`); this wires
    // it into the CLI next to `bench`/`intelligence`/`eval`.
    case "agent-bench": {
      const { runAgentBenchmark } = await import("./agent-benchmark.js")
      const { EVAL_TASKS } = await import("./evalbench.js")
      const one = typeof flags.task === "string" ? flags.task : (positional[1] && positional[1] !== "list" ? positional[1] : "")
      const tasks = one ? EVAL_TASKS.filter((t) => t.id === one) : EVAL_TASKS
      if (one && !tasks.length) { err(`unknown task: ${one}`); process.exit(2); return }
      const result = await runAgentBenchmark({ tasks })
      if (JSON_OUT) { emitJson(result); process.exit(result.status === "NOT_RUN" ? 2 : (result.falseCompletions || result.errored ? 1 : 0)); return }
      console.log(bold(`FORGE AGENT-BENCH v${VERSION}`) + dim("  live-provider harness; hidden verification is the score"))
      if (result.status === "NOT_RUN") { console.log(dim(`  NOT_RUN — ${result.reason} (${result.tasks} task(s) available)`)); process.exit(2); return }
      console.log(`  provider=${result.provider}`)
      console.log(JSON.stringify(result, null, 2))
      process.exit(result.falseCompletions || result.errored ? 1 : 0)
      return
    }
    // Deterministic, provider-free Alpha-Intelligence orchestration benchmark.
    // Also runnable standalone (`node alpha-benchmark.js`).
    case "alpha-bench": {
      const { runAlphaBenchmark } = await import("./alpha-benchmark.js")
      const result = runAlphaBenchmark({ keep: flags.keep === true })
      if (JSON_OUT) { emitJson(result); return }
      console.log(bold(`FORGE ALPHA-BENCH v${VERSION}`) + dim("  deterministic orchestration benchmark, no live model"))
      console.log(`  cases=${result.cases} failureCases=${result.failureCases} classificationRate=${result.knownFailureClassificationRate}`)
      console.log(`  persistedSamples=${result.persistedSamples} elapsed=${result.elapsedMs.toFixed(1)}ms`)
    ...
    case "selfaudit": {
      ...
      const report = analyzeModules({
        dir: target, testDir,
        // Modules with no core importer BY DESIGN: the CLI entry (forge.js), the
        // sandboxed plugin loader and the auditor itself, plus independently-
        // adoptable V4 primitives and the npm-test resource policy. Each is a
        // deliberate standalone unit (some carry their own tests / main guard),
