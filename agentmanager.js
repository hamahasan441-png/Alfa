...
export function createAgentManager({
  ...
  runner = null,
  signal = null,
  /** how long a cancelled worker may take to settle before it is orphaned */
  cancellationGraceMs = DEFAULT_GRACE_MS,
} = {}) {
  let seq = 0
  ...
  async function runWhenReady(rec, timeoutMs) {
    ...
    try {
      ...
      // --- WAIT_FOR_SHUTDOWN ---------------------------------------------
      await Promise.race([runnerPromise.catch(() => ({})), sleep(Math.max(0, Number(cancellationGraceMs) || 0))])
      if (live.has(rec.id)) {
