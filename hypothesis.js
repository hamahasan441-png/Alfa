...
export function ingestHypothesisVerificationFailure(engine, event = {}) {
  if (!engine?.add) return { hypothesis: null, reason: "no hypothesis engine" }
  const reason = String(event.reason ?? event.evidence ?? "verification failed").slice(0, 300)
  const command = event.command ? String(event.command).slice(0, 160) : "unknown verification"
  const hypothesis = engine.add({ description: `verification failure: ${reason}`, confidence: 0.55 })
  engine.recordTest(hypothesis.id, { name: `verification:${command}`, result: "fail" })
