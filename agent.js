...
export async function runAgent({ config, provider, task, extraContext = "", onEvent, signal, readOnly = false, planOnly = false, maxStepsOverride, deep, role, sub = null, journal = true, runIdOverride = null, runId: runIdParam = null, suppressRunEvents = false, keepJournalRunning = false, noTools = false, worker = null, taskId = null, segmentId = null, nodeId = null, verifier = false, pluginStartedAt = null }) {
  ...
  if (!sub && !verifier && config.agent?.cognition !== false) {
    try {
      ...
      // populated on the live one-shot path.
      cognition.boot(task)
  ...
  try {
    ...
    while (true) {
      ...
      if (msg.toolCalls?.length) {
        ...
        // v123.2: feed observed execution evidence into the long-horizon controller
        // on the live path. Advisory only: governor/authority/completion remain authoritative.
        if (cognition?.updateHorizon) {
          try {
            const horizonTests = commandChecks.slice(-32).map((c) => ({ id: c.command, passed: c.passed }))
            const failedEvidence = results.map((r) => String(r?.result ?? "")).filter(x => /^ERROR|^BLOCKED/.test(x)).slice(-16)
            const updated = cognition.updateHorizon({
              changedFiles: writesSoFar.filter(f => f !== "(shell write)"),
              historicalFailures: failedEvidence.map((message) => ({ file: "", message })),
              tests: horizonTests,
              evidence: commandChecks.filter(c => c.passed).slice(-16).map(c => `command:${c.command}:passed`),
              budget: { stepsLeft: Math.max(0, maxSteps - steps), maxParallel: 3 },
              attempt: steps,
            })
            onEvent?.({ type: "HORIZON_RISK_UPDATED", action: updated?.decision?.action, requiresEscalation: updated?.risk?.requiresEscalation === true, verification: updated?.risk?.verification || [], frontier: updated?.frontier?.frontier || [], impact: updated?.liveImpact?.impact?.map((x) => x.path).slice(0, 8) || [], replanned: !!updated?.replanned, recovery: updated?.recovery?.action || null, wave: updated?.coordinator?.waves?.[0]?.length || 0, ...identityMeta() })
          } catch (e) { swallowed("agent", "horizon intelligence update", e) }
        }
